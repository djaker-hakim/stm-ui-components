{{-- ID IS A MUST ON MULTIPLE MODEL COMPONENT --}}
...
<section 
    ...
    x-show="model"
    x-cloak
    x-on:open-model.window="model =  ($event.detail.id == id)"
    x-on:close-model.window="$event.detail.id == id ? model = false : ''"
    class="fixed inset-0 z-[999] grid h-screen w-screen place-items-center bg-black/30 ">

    <div 
    @if ($clickOutside) x-on:click.outside="model = false" @endif
